# Advent of Code Day 1

library(magrittr)

# Advent of Code Day 1

# Import Data as you like
module_masses <- c("100152", "121802", "140047", "92337", "101891", "122051", "50384", "53628", "139979", "57959", "90354", "119201", "53941", "74563", "140320", "69972", "90954", "85414", "52999", "69869", "65511", "91084", "146614", "120976", "145517", "121313", "99155", "144062", "53343", "60992", "81324", "109565", "83665", "100255", "116562", "71967", "66486", "76844", "83233", "129089", "98787", "118848", "120030", "123908", "144800", "113563", "74763", "80902", "58740", "115929", "57926", "61739", "118481", "111540", "55259", "90161", "110745", "85103", "92616", "126402", "71906", "137282", "76811", "124470", "140723", "89796", "98126", "127274", "104925", "120395", "134417", "105281", "140414", "52683", "149260", "123259", "125238", "68860", "103545", "90308", "118854", "121111", "72989", "62993", "96615", "145935", "75078", "96752", "118779", "68090", "95136", "82132", "149426", "51496", "70123", "129725", "63022", "74422", "143216", "139349")

module_masses <- as.numeric(module_masses)

# Part 1
# Calculate Fuel per mass

fuel_calculator <- function(x){
  floor(x / 3) -2
}

# Iterate that function over the vector of modules and finally get the sum

total_fuel <- purrr::map_dbl(module_masses, fuel_calculator) %>% sum()


# Part 2 
# Calculate the extra fuel

total_fuel_calculator <- function(new_fuel){
  
  while(new_fuel > 0){ # While-Loop needed because calculation has to stop if value is 0 or below
    
    new_fuel <- floor(new_fuel / 3) -2 # Apply Function from part 1
    
    if(new_fuel < 1){return(total_fuel)} # Stop the while loop if value is below 1
    
    else if(exists("total_fuel")){total_fuel <- new_fuel + total_fuel} # add extra fuel to total
    
    else{total_fuel <-  new_fuel} # Create total if it doesn't exist yet
  }
  
}

total_fuel_requirement <- purrr::map_dbl(module_masses, total_fuel_calculator) # Again iterate this function over input vector

sum(total_fuel_requirement) # Finally get the sum
